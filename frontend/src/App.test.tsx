...
describe("App", () => {
  ...
  it("shows lifted dialogue pending progress while a speech job runs", async () => {
    ...
    const controls = screen.getByRole("region", { name: "Generation Controls" })
    const pending = within(controls).getByRole("status")
    expect(pending).toHaveTextContent("Generating Dialogue")
    expect(pending).toHaveTextContent("0/2 Rows")
    expect(pending).toHaveTextContent("Segment 1: Default voice")
    expect(screen.queryByRole("list", { name: "Generation Progress" })).not.toBeInTheDocument()
