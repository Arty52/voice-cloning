...
- Sample-processing UI belongs under the `Prepare Audio` parent area, outside the Add Voice flow. Feature hooks should call `/api/sample-processing/*`, poll jobs, manage section-local elapsed time, manage result preview URLs, and save the selected result explicitly before adding it to the Voice Library. Future diarization or speaker-separation work should extend this parent area rather than rename it around one operation.
