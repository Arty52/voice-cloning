...
- `VoiceLibrary` owns local voice asset persistence, including `voicePresetId`. Without `DATABASE_URL`, the manifest-backed implementation under ignored `assets/voices/` remains the source of truth and normalizes missing or invalid preset ids to `standardNarration` before writing.
- When `DATABASE_URL` is configured, `create_app()` selects the PostgreSQL-backed voice library through the factory. It preserves the same route/service contract, imports existing `voices.json` entries idempotently, and keeps sample files under `VOICE_ASSETS_DIR`.
- Keep voice persistence behind the `VoiceLibrary` API and route/service contract. Frontend request shapes and provider adapter contracts must not depend on whether the backing store is manifest or Postgres.
- PostgreSQL access belongs behind repository and unit-of-work boundaries under `voice_cloning.persistence`. FastAPI routes should not import SQLAlchemy models or sessions directly.
- Server-side file stores must use configured backend roots such as `VOICE_ASSETS_DIR` and `GENERATED_AUDIO_STORAGE_DIR`. Resolve stored relative paths under those roots, create roots on startup, and never accept arbitrary filesystem paths from frontend requests.
- Database rows and local files must be created and deleted with an explicit consistency strategy: stage files before database insert, clean staged files on database failure, roll back database work on file failure, and add reconciliation tests when a route starts mutating both.
...
- `workflow-sections.ts` owns top-level section ids, group placement, labels, icons, stable hash fragments, and status derivation. Add future parent workflow areas or standalone features there only when the existing sections no longer fit.
...
- `VoiceStudioShell` owns the grouped desktop sidebar, mobile sheet navigation, active section header, and status badges; `StudioOverviewPanel` mirrors its Workflow and Features grouping. Section page components stay mounted and are hidden when inactive so local form, upload, preview, provider-key, archive, and in-flight generation state survives navigation.
...
- Transcript timing diagnostics follow the local-only, privacy-minimized contract in [TRANSCRIPT_TIMING_DIAGNOSTICS.md](TRANSCRIPT_TIMING_DIAGNOSTICS.md). Keep estimate inputs, lifecycle instrumentation, bounded retention, and storage validation outside presentational components. The current range is an uncalibrated file-size heuristic, not a duration-derived promise; retained estimate and terminal elapsed values exist to measure heuristic error and investigate variability before any explicit retuning decision.
...
- Provider-independent voice presets come from `/api/providers` as top-level `voicePresets`. A selected voice's `voicePresetId` initializes saved Voice Tuning through the active provider preset whose `voicePresetId` matches. If no active provider preset is mapped to that semantic id, the frontend falls back to provider `defaultValues`. Saved tuning belongs with voice selection, stays collapsed until opened, uses local draft state until confirmed Save Voice Tuning, offers Reset Changes back to saved voice metadata, and persists through the existing voice PATCH path. User-managed tuning presets are separate editable records from `/api/voice-tuning-presets` when persistence is configured, with browser-local fallback only for no-DB transition mode. Generate Speech keeps only contextual row or segment overrides, while generated-audio metadata snapshots any selected user preset so later preset edits do not rewrite archive provenance.
...
- Add Voice and range-oriented Sample Processing UI belong under the `Prepare Audio` parent area behind a lightweight workflow chooser. The first-class `Transcript` section belongs under the standalone `Features` group and is a separate full-file diarization consumer of the same backend job contract, not a voice-synthesis workflow step. `useTranscriptWorkflow` owns its latest-job pointer, reload restoration, polling, cancellation, and elapsed state; `useSpeakerTranscript` and `SpeakerTranscriptWorkspace` remain shared with Prepare Audio for names, turn assignment, corrections, exports, and selected-speaker saves. Keep these feature hooks responsible for `/api/sample-processing/*` calls while panel components stay presentational.
