...
Voice Clone Lab is a small local app, but changes should still keep clear boundaries between transport, state, and presentation. Public-repo safety is part of the architecture: API keys stay server-side, generated/runtime assets stay out of git, and UI code talks only to the local API.
...
- Smart containers and hooks own data loading, mutations, browser APIs, side effects, and derived state.
...
- Hooks should be feature scoped: voice library, metadata, generated audio storage, speech generation, recording/upload flow, and dialogs are separate responsibilities.
