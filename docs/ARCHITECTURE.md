...
- Sample-processing UI belongs outside the Add Voice flow. Feature hooks should call `/api/sample-processing/*`, poll jobs, manage section-local elapsed time, manage result preview URLs, and save the selected result explicitly before adding it to the Voice Library.
