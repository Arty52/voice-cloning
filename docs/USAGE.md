...
In `#prepare`, Process Audio defaults to Easy Prepare. Choose a saved voice or upload an audio file, then keep Easy Prepare selected for ranked candidates or select one or more advanced Workflow Stack cards before starting. Uploads support MP3, WAV, M4A, AAC, OGG, and FLAC. Easy Prepare shows Clean Voice, Trim Non-Spoken Audio, and Detect Speakers toggles; Clean Voice is available only with Demucs, Detect Speakers is available only with diarization, and Trim Non-Spoken Audio is enabled by default. Selected cards show selected-state styling, each selected operation with presets exposes its own preset control, and the running job shows Workflow Progress with Queued, Running, Complete, Error, or Canceled steps. Large local files can take several minutes; the intended local runtime budget for a 1 GB source is about 15 minutes on a capable machine.
