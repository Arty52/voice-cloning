...
The UI opens on `Overview`. The sidebar separates the core `Workflow` from standalone `Features`:
...
`Features` currently contains `Transcript` (`#transcript`), a standalone capability for transcribing complete local audio, editing the result, exporting text, and saving selected speakers as voices. It is not a voice-synthesis workflow step.
...
The backend never returns key material from `.env` or browser headers. Browser `localStorage` is local developer-tool storage, not encrypted secret storage; clear the key in `Provider & Usage` or browser site data to remove a saved GUI key. When backend persistence is configured, non-secret app preferences such as selected model, Natural Handoffs, and generated-audio storage limit can be stored in Postgres; provider API keys are excluded from that settings API.

Voice samples and their local manifest metadata are stored under `assets/voices/` and are ignored by git. When a long upload is saved with its original source retained, the backend stores the original under `assets/voices/sources/` and still sends only the active excerpt sample to the provider. Process Source Media staged sources and preview clips are runtime-only under ignored `storage/sample-processing/sources/` and are deleted when replaced or after the job is accepted. Cloned voice cache data is written under `storage/`, scoped by provider and key fingerprint, and ignored by git. Multi-voice speech job segment and combined audio files are runtime-only under ignored `storage/speech-jobs/`. Generated MP3 output is saved to the server archive when `DATABASE_URL` is configured, with files under `GENERATED_AUDIO_STORAGE_DIR`; browser IndexedDB remains the local fallback and migration source. Server export mirrors canonical archive files only to backend-owned `GENERATED_AUDIO_EXPORT_DIR`. Browser folder export is separate: the UI stores a File System Access directory handle in IndexedDB, rechecks write permission before mirroring, and never sends the selected local path to the backend.
...
Cropped excerpts are encoded in the browser as mono 16 kHz WAV files so a two-minute active sample stays below the default 10 MB active upload cap. The backend normalizes newly saved active samples to mono 16 kHz WAV before storing them under `assets/voices/` and sends only that active sample to the provider for cloning. When the UI sends `sampleMode=sourceWindow`, the original `sourceFile` is retained locally under `assets/voices/sources/`; it remains ignored by git, is capped by `MAX_SOURCE_UPLOAD_BYTES` which defaults to 1 GB, and is not sent to the provider.
...
Set `SAMPLE_PROCESSING_ENGINE=ffmpeg` to enable Trim Silence and ranked Prepare Voice. Set `SAMPLE_PROCESSING_ENGINE=demucs` to enable Isolate Voice, Trim Silence, and Prepare Voice with optional cleaning. Set `SAMPLE_PROCESSING_ENABLE_DIARIZATION=1` to add Speaker Separation and per-speaker Prepare Voice candidates alongside either engine, or by itself when `SAMPLE_PROCESSING_ENGINE` is blank. When Demucs is enabled, Isolate Voice runs the configured Demucs command with the `htdemucs` model by default, extracts the vocals stem, then runs FFmpeg to normalize the result to mono 16 kHz WAV. Trim Silence runs FFmpeg `silenceremove`, trims leading, trailing, and long interior empty sections, then normalizes the result to mono 16 kHz WAV. Speaker Separation normalizes the source with FFmpeg, runs pyannote Community-1 locally for speaker turns, runs faster-whisper locally with word timestamps, maps transcript text to speakers by time overlap, then generates one mono 16 kHz WAV per speaker. Existing saved voices show Process From choices. Original Recording is recommended when `sourceFilePath` exists and uses the retained full upload/source file. If no retained original exists, Original Recording is unavailable and Saved Sample is used instead. Choose Saved Sample to process the current library sample. Uploaded files can also be processed without first saving them as voices.

Prepare Voice streams large sources to disk, optionally runs Isolate Voice, optionally runs Speaker Separation, detects nonsilent speech on the cleaned/intermediate source, ranks voice-dense windows up to 120 seconds, runs final Trim Silence-style cleanup, and outputs mono 16 kHz WAV candidates. Candidate saves go through the same Voice Library path as processed samples and include prepare metadata.
...
In `#prepare`, Process Source Media defaults to Easy Prepare. Choose a saved voice, upload an audio file, or upload a video file, then keep Easy Prepare selected for ranked candidates or select one or more advanced Workflow Stack cards before starting. Audio File uploads support MP3, WAV, M4A, M4B, AAC, OGG, and FLAC. Video File uploads support local `.mp4`, `.m4v`, and `.mov` sources with `video/mp4`, `video/x-m4v`, or `video/quicktime`; WebM, MKV, AVI, WMV, FLV, TS, MTS, and M2TS are deferred. Add Voice still expects browser-decodable ready samples for its crop preview path, so use Process Source Media for audiobook-style M4B sources or local video clips that need speech extraction.

When you upload source media, the backend stages and inspects it before job creation. Audio sources with chapter metadata show a Source Selection list: choose one or more chapters, optionally play bounded preview clips, and start processing only after at least one chapter is selected. Sources without chapters show a manual range slider with start/end time, selected duration, and preview playback. Video sources show the browser's native video controls when the staged media is playable in the current browser, plus the same Source Selection range controls. The backend records available audio stream metadata and extracts from the first audio stream by default; track selection UI is deferred. The selected chapter or range is extracted to mono 16 kHz WAV for processing; unselected parts of a large file are not fed into the job. Staged sources are cleaned up when you replace the upload or after the job is accepted.

Easy Prepare shows Clean Voice, Trim Non-Spoken Audio, and Detect Speakers toggles; Clean Voice is available only with Demucs, Detect Speakers is available only with diarization, and Trim Non-Spoken Audio is enabled by default. Selected cards show selected-state styling, each selected operation with presets exposes its own preset control, and the running job shows Workflow Progress with Queued, Running, Complete, Error, or Canceled steps. Large local files can take several minutes; selecting only the needed chapters or range keeps processing bounded. The intended local runtime budget for a 1 GB selected source is about 15 minutes on a capable machine.
...
Processed results are candidates, not automatic voice-library entries. Sample Processing never refines, overwrites, or replaces the selected saved voice. Preview the result first, then choose Add To Voice Library to store it under `assets/voices/` through the same local Voice Library path as uploaded samples. This creates a new voice whose `filePath`, `contentType`, and `sha256` point at the processed active sample, and includes `processingSteps` metadata for traceability, including every completed stack step and selected preset when one was used. Speaker Separation saves any selected speaker streams as independent voices, each with its own Voice Name and Voice Preset selection, and keeps prior stack steps plus speaker-specific split or trim metadata. Streams within both 120 seconds and the active provider byte cap are saved in full. For longer or larger streams, the backend automatically ranks speech-dense windows and saves the best mono 16 kHz excerpt within both limits, recording a final Prepare Voice processing step. All selected speakers are prepared before any are added, so a failed multi-speaker preparation or save is rolled back as one batch. The Voice Preset options come from `/api/providers.voicePresets`, the same semantic preset source used by normal voice uploads.
...
Runtime files are written under ignored `storage/sample-processing/`. Staged Process Source Media uploads live under `storage/sample-processing/sources/` until replacement or job creation cleanup. The full staged upload is capped by `MAX_SOURCE_UPLOAD_BYTES`; the selected extracted mono 16 kHz audio fed into a job is capped separately by `MAX_SELECTED_SOURCE_AUDIO_BYTES`, which defaults to the source upload cap. Demucs output folders, diarization transcripts, full generated speaker preview streams, provider-safe speaker excerpts, and intermediate stems are job-local. Full speaker preview streams use the trusted `MAX_SOURCE_UPLOAD_BYTES` cap rather than the smaller provider-facing `MAX_UPLOAD_BYTES`; only the excerpt copied into the Voice Library must fit the provider-facing limit. Heavy model files are runtime data; the Docker stack routes them to ignored `storage/model-cache/`. For offline use, run once while the machine can reach Hugging Face and faster-whisper model storage so the model files are present in the local cache before disconnecting.
