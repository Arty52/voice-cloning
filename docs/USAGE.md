...
In `#prepare`, select one or more Workflow Stack cards before starting. Selected cards show their stack step number, each selected operation with presets exposes its own preset control, and the running job shows Workflow Progress with active, queued, completed, failed, or canceled steps.
