...
4. `Generated Audio` (`#archive`, optional): play, download, remove, or clear saved generated MP3s. With backend persistence configured, the archive streams from the server; otherwise it falls back to browser IndexedDB. Multi-Voice archive metadata is preserved in both modes.
