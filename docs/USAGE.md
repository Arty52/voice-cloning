...
`Prepare Audio` is a separate optional workflow from `Voices`. Start with Add Voice when the sample is ready to save, or Process Audio when the source needs cleanup before it becomes a library voice. Current processing operations are Prepare Voice, Isolate Voice, Trim Silence, and optional Speaker Separation.
...
In `#prepare`, Process Audio defaults to Easy Prepare. Choose a saved voice or upload an audio file, then keep Easy Prepare selected for ranked candidates or select one or more advanced Workflow Stack cards before starting. Process Audio uploads support MP3, WAV, M4A, M4B, AAC, OGG, and FLAC. Add Voice still expects browser-decodable uploads for its crop preview path, so use Process Audio for audiobook-style M4B sources.

When you upload a Process Audio file, the backend stages and inspects it before job creation. Audiobooks with chapter metadata show a Source Selection list: choose one or more chapters, optionally play bounded preview clips, and start processing only after at least one chapter is selected. Files without chapters show a manual range slider with start/end time, selected duration, and preview playback. The selected chapter or range is extracted to mono 16 kHz WAV for processing; unselected parts of a large file are not fed into the job. Staged sources are cleaned up when you replace the upload or after the job is accepted.
