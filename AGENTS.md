...
- Keep `ELEVENLABS_API_KEY` server-side. Browser-entered developer keys may be stored only in browser-local storage; browser code sends them only to the local API through the provider key request header, and the backend may use them per request to authenticate provider calls. Never expose `.env` key material to frontend responses.
